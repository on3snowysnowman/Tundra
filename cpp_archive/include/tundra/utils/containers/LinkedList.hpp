...
static constexpr uint64 SENTINEL_IDX = 0;
...
template<typename T>
inline void default_copy(const T *src, T *dst)
{
    // Simple byte copy
    *dst = *src;
}
...
template<typename T>
inline void default_free(T* /** datum */)
{
    // Placeholder method for a a function pointer, never called.
}
...
inline void init(LinkedList<T> &list,
    void (*copy_func)(const T*, T*) = Internal::default_copy,
    void (*free_func)(T*) = Internal::default_free)
{
...
inline void copy(const LinkedList<T> &src, LinkedList<T> &dst)
{
    ...
    uint64 idx = src.head_idx;

    while(idx != Internal::SENTINEL_IDX)
    {
        src.copy_func(src.nodes[idx].datum, dst.nodes[idx].datum);
