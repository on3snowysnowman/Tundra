...
usage() 
{
  cat <<'EOF'
  Usage:
    gen_Stk.sh --type <type> --typename <name> [-o DIR] [-f]
  ...
  Examples:
    gen_Stk.sh --type uint32 --typename u32
    gen_Stk.sh --type MyStruct* --typename MyStruct_ptr -o include/containers
    gen_Stk.sh --type Foo --typename Foo --custom-copy --custom-free -f
    gen_Stk.sh --type Bar --typename Bar --custom-all
EOF
}
...
OUT_PATH="${OUT_DIR%/}/Stack${TYPENAME}.h"
...
GENERATED_OUTPUT="#ifndef TUNDRA_STACK${TYPENAME}_H
#define TUNDRA_STACK${TYPENAME}_H
...
GENERATED_OUTPUT+=\
"
#endif // TUNDRA_STACK${TYPENAME}_H
"
