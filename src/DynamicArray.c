...
/**
 * @brief Resizes and reallocates the Array's data.
 * 
 * The new capacity of the Array will be double its current capacity.
 * 
 * @param array Array to resize.
 */
static void resize_array(Tundra_DynamicArray* array)
{
    // Allocate new block of memory that is twice the current capacity.
    void* new_memory = malloc(array->data_type_size * array->capacity * 2);

    // Copy old elements into the new memory.
    memcpy(new_memory, array->data, array->capacity * array->data_type_size);
...
{
    if(array->num_elements == array->capacity) resize_array(array);
}
...
{
    array->data = malloc(data_type_size * starting_capacity);
