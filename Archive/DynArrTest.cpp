...
TEST_BEGIN(add)
{
    ...
    Tundra_DynArrint_add_by_init(&arr, 1);

    assert(arr.data[0] == 1);
    assert(arr.cap == 2);

    Tundra_DynArrint_add_by_init(&arr, 2);

    assert(arr.data[1] == 2);
    assert(arr.cap == 2);

    Tundra_DynArrint_add_by_init(&arr, 3);
...
TEST_BEGIN(add_multiple)
{
    ...
    Tundra_DynArrint_add_by_init_multiple(&arr, elems, NUM_INIT_ELEM);
